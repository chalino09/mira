-- P0: una ejecución nunca equivale a una verificación.
-- Mantiene la protección también para adaptadores heredados y Telegram.
...
as $$
begin
  if old.status <> 'completada'::public.task_status
    and new.status = 'verificada'::public.task_status then
...
create trigger tasks_prevent_automatic_verification
before update of status on public.tasks
for each row
execute function public.prevent_automatic_work_verification();
...
begin
  if new.update_type = 'verified'::public.task_update_type
    and coalesce((new.metadata ->> 'automatic')::boolean, false) then
    return null;
  end if;
  return new;
end;
...
create trigger task_updates_skip_automatic_verification
before insert on public.task_updates
for each row
execute function public.skip_automatic_work_verification_event();
...
begin
  ...
  select actor_user_id into completed_by
  ...
  insert into public.task_updates (company_id, task_id, actor_user_id, update_type, note, metadata)
  values (target_work.company_id, target_work.id, auth.uid(), 'verified', note_value,
...
declare
  target_work public.tasks%rowtype;
  completion_event public.task_updates%rowtype;
begin
  select * into target_work from public.tasks where id = target_work_id for update;
  if target_work.id is null then raise exception 'task_not_found'; end if;
  if target_work.type in ('riego'::public.task_type, 'fertirriego'::public.task_type,
    'fertilizacion'::public.task_type, 'aplicacion_foliar'::public.task_type,
    'cosecha'::public.task_type) then raise exception 'technical_work_requires_reopen'; end if;
  if target_work.status <> 'completada'::public.task_status then raise exception 'invalid_work_transition'; end if;
  select * into completion_event from public.task_updates
  where task_id = target_work_id and update_type = 'completed'::public.task_update_type
  order by created_at desc limit 1;
  if completion_event.actor_user_id <> auth.uid() or completion_event.created_at < now() - interval '30 seconds' then
    raise exception 'work_completion_undo_expired';
  end if;

  update public.tasks set status = 'en_progreso', completed_at = null, occurred_at = null,
    verified_at = null, verified_by = null, updated_at = now()
  where id = target_work_id returning * into target_work;
  insert into public.task_updates (company_id, task_id, actor_user_id, update_type, note, metadata)
  values (target_work.company_id, target_work.id, auth.uid(), 'reopened', 'Se deshizo la finalización.',
    jsonb_build_object('source', 'work', 'undo', true));
  return jsonb_build_object('workId', target_work.id, 'status', target_work.status);
end;
$$;
