-- Allow credit accounts to remain open without a committed due date.
alter table public.nursery_sales
  drop constraint if exists nursery_sales_credit_due_date_required;

alter table public.nursery_sales
  add constraint nursery_sales_credit_due_date_valid check (
    (payment_terms = 'credit' and (due_date is null or due_date >= occurred_at))
...
begin
  ...
  if target_payment_terms = 'credit' and target_due_date is not null and target_due_date < target_occurred_at then
    raise exception 'nursery_sale_credit_due_date_invalid';
  end if;
