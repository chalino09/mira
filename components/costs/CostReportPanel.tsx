...
import { SelectInput } from "@/components/forms/FormControls";
import {
  compareCostRecords,
        ...
        <div className="border-t border-app-border p-4 sm:p-5" id={reportContentId}>
          ...
          <div className="mt-5 rounded-xl bg-app-sidebar/60 p-4">
            ...
            <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <label className="grid min-w-0 gap-1.5 text-xs font-medium text-app-text">Desde<DatePickerInput aria-label="Fecha inicial del reporte" className="h-10" showQuickActions={false} value={filters.startDate} onChange={(event) => updateFilter("startDate", event.target.value)} /></label>
              <label className="grid min-w-0 gap-1.5 text-xs font-medium text-app-text">Hasta<DatePickerInput aria-label="Fecha final del reporte" className="h-10" showQuickActions={false} value={filters.endDate} onChange={(event) => updateFilter("endDate", event.target.value)} /></label>
              <label className="grid min-w-0 gap-1.5 text-xs font-medium text-app-text">Categoría<SelectInput aria-label="Categoría del reporte" className="h-10" value={filters.category} onChange={(event) => updateFilter("category", event.target.value)}><option value="">Todas</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</SelectInput></label>
              <label className="grid min-w-0 gap-1.5 text-xs font-medium text-app-text">Módulo<SelectInput aria-label="Módulo del reporte" className="h-10" value={filters.sector} onChange={(event) => updateFilter("sector", event.target.value)}><option value="">Todos</option><option value="General / sin módulo">General / sin módulo</option>{sectorOptions.filter((sector) => sector !== "General / sin módulo").map((sector) => <option key={sector} value={sector}>{sector}</option>)}</SelectInput></label>
              <label className="grid min-w-0 gap-1.5 text-xs font-medium text-app-text">Origen<SelectInput aria-label="Origen del reporte" className="h-10" value={filters.origin} onChange={(event) => updateFilter("origin", event.target.value)}><option value="">Todos</option>{origins.map((origin) => <option key={origin} value={origin}>{originLabel(origin)}</option>)}</SelectInput></label>
